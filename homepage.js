...
function init() {
  ...
  if (isWebgl()) {
    ...
    renderer.setClearAlpha(0); // Clear alpha buffer
    renderer.domElement.style.backgroundColor = "transparent"; // Set background color to transparent
  } else {
  ...
  var meshArray = [];
  var color;
  // Define a scale factor for shrinking the objects
  var scaleFactor = 0.5; // Adjust this value as needed

  // Adjust the size of the geometry
  var geometry = new THREE.BoxGeometry(
      DOT_SIZE * 0.8 * scaleFactor, // Adjusting width
      DOT_SIZE * 0.8 * scaleFactor, // Adjusting height
      DOT_SIZE * 0.8 * scaleFactor  // Adjusting depth
  );
...
function onDocumentTouchMove(event) {
  if (event.touches.length === 1) {
    ...
    targetRotation =
      targetRotationOnMouseDown + (mouseX - mouseXOnMouseDown) * rotationSpeed;
  }
}
...
function changeFormation1() {
  for (var i = 0; i < list.length; i++) {
    ...
    new TWEEN.Tween(list[i].position)
      .to(
        {
          x: vx,
          y: vy,
    ...
    new TWEEN.Tween(list[i].rotation)
      .to(
        {
          x: 0,
          y: rot,
...
function changeFormation2() {
  ...
  for (j = 0; j < dataSet.length; j++) {
    for (i = 0; i < dataSet[j].length; i++) {
      ...
      z = j * DOT_SIZE + Z_START_POS;
      if (dataSet[j][i] != "BK") {
        new TWEEN.Tween(list[k].position)
          .to(
            {
              x: x,
              y: y,
        ...
        new TWEEN.Tween(list[k].rotation)
          .to(
            {
              x: 0,
              y: 0,
